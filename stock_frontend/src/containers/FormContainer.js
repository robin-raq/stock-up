...
export default class FormContainer extends Component {
  ...
  handleSubmit = (evt) => {
    evt.preventDefault();
    fetch(
      `https://cloud.iexapis.com/stable/stock/${this.state.ticker}/quote?token=${tokens.IEX_TOKEN}`
    )
      .then((r) => r.json())
      .then((stockObj) => {
        ...
        const canBuy =
          parseFloat(transaction.total) <= parseInt(this.state.balance);
            ...
            <legend className="pa0 f5 f4-ns mb3 black-80">
              {this.state.balance >= 0 ? (
                <strong className="text-">
                  Current Balance: ${parseFloat(this.state.balance).toFixed(2)}
                </strong>
              ) : (
              ...
              <>
                <span>
                  ...
                  <p>
                    <Pluralize
                      singular={"share"}
                      count={parseInt(this.state.quantity)}
                    />
                    &nbsp; of {this.state.ticker} stock @ ${price.toFixed(2)}{" "}
                    per share
                  </p>
