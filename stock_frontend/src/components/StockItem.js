...
export default class StockItem extends Component {
  ...
  getStockInfo = (ticker) => {
    ...
    fetch(
      `https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol=${ticker}&apikey=${tokens.AlPHAV_TOKEN}`
    )
      .then((r) => r.json())
      .then((stockObj) => {
        let stockTotal =
          parseFloat(stockObj["Global Quote"]["05. price"]) *
          parseInt(this.props.stock.quantity);
        const alphaStock = {
        ...
        /* 
            use code below to use IEX API instead
                const IEXStock = {
      ...
      .catch((error) => {
        alert(
          "We were unable to connect with the market server, click Ok to try again"
        );
      });
  };
        ...
        <div className="card avenir" style={{ background: this.stockColor() }}>
          <div className="card-body">
            ...
            <p className="card-text">
              ...
              {(
                this.state.marketStock.price * this.props.stock.quantity
              ).toFixed(2)}
            </p>
          </div>
        </div>
